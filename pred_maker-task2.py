...
train_df = pd.read_csv('../tsv/final/task2/train.tsv','\t')
dev_df = pd.read_csv('../tsv/final/dev.tsv',sep='\t')
...
num_of_epochs = 50
learning_rate = 0.001
model = TuningNet(300,512,100).to(device)
train_batch_size = 32


model = torch.load('../semi-supervised_train/models/task2-lvl00.pt').to(device)
...
#     #         y_true += list(batch['label'].argmax(dim=1).detach().cpu().numpy())
    f = open('../tsv/test_pred-task2-max.txt','wt')
    with open('../tsv/test_pred-task2.tsv', 'wt') as out_file:
        tsv_writer = csv.writer(out_file, delimiter='\t')
        tsv_writer.writerow(['#1 tweet_ID', '#4 province_label', 'conf'])
